...
/* Input Parameters: struct LINKEDLISTNODE** - to headref (starting node of linked list), uint32_t data - data to be allocated to new node, uint32 position - index at which to add node
   ...
   */
status add_node(node** headref, uint32_t new_data, uint32_t position)
{
...
*/
status remove_node(node** headref, uint32_t node_position, uint32_t* data_removed)
{
...
/* Input Parameters: struct LINKEDLISTNODE** - to headref (starting node of linked list), uint32_t search_for - data to be searched for, uint32_t* index - used to pass index of found node back to function
   ...
   */
node* search_node(node** headref, uint32_t search_for, uint32_t* index)
{
    node* current = *headref;
    uint32_t position = 0;
    while (current != NULL)
    ...
    return *headref; //returns default headref pointer if required data isnt found
    *index = node_not_found;
}
...
status print_list (node** headref)
{
    ...
    while (current != NULL)
    {
        printf("\nelement: %d", current->data); //prints out corresponding element
        current = current->next; //traverses the list
    }
    printf("\nLength of the List is: %d" , length); //prints final length of the list
    return success;
